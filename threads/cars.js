...
bot.on('ready', ()=>{
	console.log("**CARS THREAD** is now Online.");
	bot.channels.get(botChans.botChannelId).send("**CARS THREAD** is now **Online.**");
});

// Thread console heartbeat
const threadHB = function sendHB(){
	console.log("**CARS THREAD** is ACTIVE");
};
...
bot.on("message", message => {
  ...
  try {
    let commandFile = require(`../cars/${command}.js`);
